...
void test_succ_pred(TTree **tree)
{
	...
	value = 3;
	ASSERT(f, *((long *)successor(search((*tree), (*tree)->root, &value))->elem) == 4l, "Successor-03 ");

	value = 4;
	ASSERT(f, *((long *)successor(search((*tree), (*tree)->root, &value))->elem) == 5l, "Successor-04 ");
	...
	value = 3;
	ASSERT(f, *((long *)predecessor(search((*tree), (*tree)->root, &value))->elem) == 2l, "Predecesor-03");

	value = 6;
	ASSERT(f, *((long *)predecessor(search((*tree), (*tree)->root, &value))->elem) == 5l, "Predecesor-04");
